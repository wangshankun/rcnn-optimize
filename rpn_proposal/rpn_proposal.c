...
int rpn_proposal()
{
    delta_box* delta_boxs  = calloc(9*50*38, sizeof(delta_box));
    box*       pre_boxs    = calloc(6000,    sizeof(box));
    anchor*  top_data  = calloc(300, sizeof(anchor));
    u_float* top_score = calloc(300, sizeof(u_float));
    
    anchor*  anchor_buf    = calloc(9*50*38, sizeof(anchor));
    delta*   delta_buf     = calloc(9*50*38, sizeof(delta));
    u_float* score_buf     = calloc(9*50*38, sizeof(u_float));  
    ...
    printf("fd_anchor read size:%d \r\n", fread(anchor_buf, sizeof(anchor),  9*50*38,  fd_anchor));
    printf("fd_delta read size:%d  \r\n", fread(delta_buf,  sizeof(delta),   9*50*38,  fd_delta));    
    printf("fd_score read size:%d  \r\n", fread(score_buf,  sizeof(u_float), 9*50*38,  fd_score));
    ...
    //        printf("%f %f %f %f %f %f %f %f %f\r\n",delta_boxs[i].x1, delta_boxs[i].y1, delta_boxs[i].x2, delta_boxs[i].y2, \
        ...
        qsort(delta_boxs, 9*50*38, sizeof(delta_box), nms_comparator);
        ...
        for(i = 0; i < 9*50*38; i++)
        {
            ...
            if((tmp_w >= min_size) & (tmp_h >= min_size))
            {
                if(k < 6000)
                {
                    ...
                    printf("i:%d k:%d %f %f %f %f %f\r\n",i,k,pre_boxs[k].x1, pre_boxs[k].y1, pre_boxs[k].x2, pre_boxs[k].y2, pre_boxs[k].score);
                    k++;
                }
                else
                {
        ...
        for(i = 0; i < 6000; i++)
        {
            ...
            printf("i:%d sc:%d %f %f %f %f %f\r\n",i,sc,pre_boxs[i].x1, pre_boxs[i].y1, pre_boxs[i].x2, pre_boxs[i].y2, pre_boxs[i].score);
            if(sc == 300) break;
        }
        finish=clock();
    ...
    printf("test count:%d used times:%f\r\n",test_count, int_time);
