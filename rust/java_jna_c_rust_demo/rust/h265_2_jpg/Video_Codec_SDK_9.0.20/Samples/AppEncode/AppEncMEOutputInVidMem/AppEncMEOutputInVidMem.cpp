...
void *AllocateHostBuffers(CUcontext device, uint32_t size)
{
    void *pDeviceFrame;
    CUDA_DRVAPI_CALL(cuCtxPushCurrent(device));

    CUresult cuResult = cuMemAllocHost((void **)&pDeviceFrame, size);
    if (cuResult != CUDA_SUCCESS)
    {
        NVENC_THROW_ERROR("cuMemAllocHost Failed", NV_ENC_ERR_OUT_OF_MEMORY);
    }
...
void ReleaseHostBuffers(CUcontext cuContext, NV_ENC_OUTPUT_PTR pHostMemoryBuffer)
{
    CUDA_DRVAPI_CALL(cuCtxPushCurrent(cuContext));

    cuMemFreeHost(reinterpret_cast<void *>(pHostMemoryBuffer));
    
    CUDA_DRVAPI_CALL(cuCtxPopCurrent(NULL));
}

void CopyOutputInHostMemory(std::vector<uint8_t> &vPacket, NV_ENC_OUTPUT_PTR pVideoMemoryBuffer, CUcontext device, NV_ENC_OUTPUT_PTR pHostMem, uint32_t bfrSize)
{
    if (pVideoMemoryBuffer != NULL)
    {
        CUDA_DRVAPI_CALL(cuCtxPushCurrent(device));

        CUresult cuResult = cuMemcpyDtoH(pHostMem, (CUdeviceptr)(pVideoMemoryBuffer), bfrSize);
        if (cuResult != CUDA_SUCCESS)
        {
            NVENC_THROW_ERROR("cuMemcpyDtoH Failed", NV_ENC_ERR_INVALID_PTR);
        }
        CUDA_DRVAPI_CALL(cuCtxPopCurrent(NULL));

        uint8_t *pMEOutput = ((uint8_t *)pHostMem);
       
        vPacket.clear();

        vPacket.insert(vPacket.end(), &pMEOutput[0], &pMEOutput[bfrSize]);
    }
}

void MotionEstimationWithBufferedFile(NvEncoderOutputInVidMemCuda *pEnc, int nWidth, int nHeight, NvEncoderInitParam *pInitParam,
    char *szInFilePath, char *szOutFilePath, uint32_t nFrame)
{
    ...
    NV_ENC_OUTPUT_PTR pHostMemoryBuffer = nullptr;
    uint32_t bufferSize = pEnc->GetOutputBufferSize();

    pHostMemoryBuffer = (NV_ENC_OUTPUT_PTR)(AllocateHostBuffers(reinterpret_cast<CUcontext>(pEnc->GetDevice()), bufferSize));
    ...
    for (uint32_t i = 0; i < nFrame - 1; i++)
    {
        ...
        pEnc->RunMotionEstimation(pVideoMemBfr);

        if (pVideoMemBfr.size() != 0)
        {
            CopyOutputInHostMemory(mvData, pVideoMemBfr[0], reinterpret_cast<CUcontext>(pEnc->GetDevice()), pHostMemoryBuffer, bufferSize);
        }
    ...
    ReleaseHostBuffers(reinterpret_cast<CUcontext>(pEnc->GetDevice()), pHostMemoryBuffer);
...
void ShowHelpAndExit(const char *szBadOption = NULL)
{
    ...
    oss << "Options:" << std::endl
        ...
        ;
    oss << NvEncoderInitParam().GetHelpMessage(true, false, false, true);
    if (bThrowError)
    {
...
*
*  The application uses the CUDA device type when demonstrating this feature 
*  for H264 ME-only mode.
*
*  This feature can also be used for H264 ME-only mode with D3D11 device type. 
*  This feature can also be used for H264 and HEVC encode with CUDA and D3D11 device types.
*
...
int main(int argc, char **argv)
{
    ...
    try
    {
        using NvEncCudaPtr = std::unique_ptr<NvEncoderOutputInVidMemCuda, std::function<void(NvEncoderOutputInVidMemCuda*)>>;
        auto EncodeDeleteFunc = [](NvEncoderOutputInVidMemCuda *pEnc)
        {
            if (pEnc)
            {
                pEnc->DestroyEncoder();
        ...
        NvEncCudaPtr pEnc(new NvEncoderOutputInVidMemCuda(cuContext, nWidth, nHeight, eFormat, true), EncodeDeleteFunc);
