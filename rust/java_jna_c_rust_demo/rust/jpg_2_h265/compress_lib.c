...
int compress_2_h265(unsigned char* in_datas, int* sizes, int in_num, unsigned char** out_data, int *out_size)
{
    int ret, i, got_output;
    JPEGINFO_t jpeg_info;
    ret = JPEG_HeaderParser(&jpeg_info ,in_datas, sizes[0]); 
    if (ret < 0)
    {
    ...
    int width  = jpeg_info.uiWidth;
    int height = jpeg_info.uiHeight; 
    ...
    av_dict_set(&param, "qp", "37", 0);//-1 默认压缩10倍、37压缩20倍、42压缩40倍、47压缩50倍
